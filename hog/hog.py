...
def is_always_roll(strategy, goal=GOAL):
    ...
    # END PROBLEM 7
    cur,opponent = 0,0
    cnt=strategy(cur,opponent)
    for cur in range(goal):
        for opponent in range(goal):
            if strategy(cur,opponent)!=cnt:
                return False
    return True
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    difference1=boar_brawl(score,opponent_score)
    cnt=score+difference1
    if sus_points(cnt)-score>=threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
